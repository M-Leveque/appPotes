...
class PhotoDAO extends DAO{
  ...
  public function get($id){
    ...
    $stmt->bindValue(':id', $id, PDO::FETCH_OBJ);
    $stmt->execute();
  ...
  public function set($photo){
    ...
    //requete SQL
    $stmt = $this->cnx->prepare("UPDATE Photo SET Titre_P = :titre, Chemin_P = :chemin,
                                    Compteur_P = :compteur, Date_P = :date, DateU_P = :dateU, Id_User = :idU; Id_Album = :IdA");
    $stmt->bindValue(':titre', $photo->getTitre(), PDO::FETCH_OBJ);
    $stmt->bindValue(':chemin', $photo->getChemin(), PDO::FETCH_OBJ);
    $stmt->bindValue(':compteur', $photo->getCompteur(), PDO::FETCH_OBJ);
    $stmt->bindValue(':date', $photo->getDate(), PDO::FETCH_OBJ);
    $stmt->bindValue(':dateU', $photo->getDateU(), PDO::FETCH_OBJ);
    $stmt->bindValue(':idU', $photo->getUtilisateur()->getId(), PDO::FETCH_OBJ);
    $stmt->bindValue(':idA', $photo->getAlbum()->getId(), PDO::FETCH_OBJ);
    $stmt->execute();
  }
  ...
  public function add($photo){
    ...
    //requete SQL
    $stmt = $this->cnx->prepare("INSERT INTO Photo(Id_P, Titre_P, Chemin_P, Compteur_P, Date_P, DateU_P, Id_User, Id_Album)
                                  VALUES (:id ,:titre, :chemin, :compteur, :date, :dateU, :idU, :idA)");
    $stmt->bindValue(':id', $photo->getId(), PDO::FETCH_OBJ);
    $stmt->bindValue(':titre', $photo->getTitre(), PDO::FETCH_OBJ);
    $stmt->bindValue(':chemin', $photo->getChemin(), PDO::FETCH_OBJ);
    $stmt->bindValue(':compteur', $photo->getCompteur(), PDO::FETCH_OBJ);
    $stmt->bindValue(':date', $photo->getDate(), PDO::FETCH_OBJ);
  ...
  public function delete($id){
    ...
    $stmt->bindValue(':id', $id, FETCH_OBJ);
    $stmt->execute();
  }
}
